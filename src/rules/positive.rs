...
{
    fn validate_positive(&self, msg: Option<String>) -> Result<()> {
        if *self <= Self::zero() {
            return Err(msg.map(Error::Custom).unwrap_or(Error::Positive));
        }
        Ok(())
    }
...
macro_rules! validate_numbers {
    ($type:ty) => {
        impl ValidatePositive for $type {
            fn zero() -> Self {
...
#[cfg(test)]
mod test {
    use super::ValidatePositive;

    #[test]
    fn test_validate_number() {
        assert!(3.validate_positive(None).is_ok());
    }
}
